...
| `GET /app[/…]` | Session cookie, or an operator token entered on the sign-in page | The new web app (preview): sign-in, console shell, the standups list and the guided setup at `/app/setup` (Cloud project → service account → Chat app with live verification → sign-in → first standup from a template, run immediately); the remaining screens land release by release while `/dashboard` stays |
