...
1. **Bootstrap** — a handful of environment variables (where's the database,
   what port, the operator token). Set once, rarely touched.
2. **Everything else** — managed in the **[web app](./web-app)
...
| --- | --- | --- |
| `PORT` | `8080` | Webhook, API and web app port |
...
| `OPERATOR_TOKEN` | *(empty)* | Operator token for the web app and the JSON API — the way in before Google or SAML sign-in is configured. Disabled while empty. `DASHBOARD_TOKEN` is still accepted as the same setting |
...
| OAuth client ID / secret | Enables [Sign in with Google](./web-app#signing-in): Workspace admins get the admin console, everyone else the `/me` user console |
...
| `GET /app` | [Web app](./web-app) — setup, console, settings |
| `GET /app/me` | [Personal page](./web-app) — own standups + self-service |
...
Give a standup a **Webhook URL** (Standup › Settings › Integrations) and AsyncUp POSTs
...
signing secret. The secret is shown in the standup's settings (derived
