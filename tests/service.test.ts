...
describe('StandupService', () => {
  ...
  it('opens blockers from blocker answers and auto-resolves on the next clean submission', async () => {
    ...
    expect((await repo.listOpenBlockers(standup.id)).map((b) => b.text)).toEqual(['Waiting on API keys']);
  ...
  it('re-derives blockers on edit without resolving older ones', async () => {
    ...
    expect((await repo.listOpenBlockers(standup.id)).map((b) => b.text)).toEqual(['Blocker B']);
  ...
  it('skips today only before submitting', async () => {
    const { repo, service } = await makeStack();
    const standup = await seedStandup(repo);
    const run = await repo.createRun(standup.id, '2026-06-10', 'key');

    expect(await service.skipToday(run.id, 'users/alice')).toBe('skipped');
    expect(
      (await repo.listRunParticipants(run.id)).find((p) => p.userName === 'users/alice')?.skippedAt,
    ).not.toBeNull();
    expect(await service.skipToday(999, 'users/alice')).toBe('not_found');
  });
  ...
  it('prefills yesterday from the previous today, and full answers when editing', async () => {
    ...
    const run2 = await repo.createRun(standup.id, '2026-06-10', 'k2');
    expect(await service.getPrefill(standup, run2, 'users/alice')).toEqual([
      'Start billing webhooks', // previous "today"
      '',
      '',
    ]);
    expect(await service.getPrefill(standup, run2, 'users/bob')).toEqual(['', '', '']);
  ...
  it('builds summaries with away/skip handling and open blockers', async () => {
    const { repo, service } = await makeStack();
    const standup = await seedStandup(repo);
    await repo.upsertParticipant({ standupId: standup.id, userName: 'users/dave', displayName: 'Dave' });
    await repo.setParticipantVacation(standup.id, 'users/dave', true);
    const run = await repo.createRun(standup.id, '2026-06-10', 'key');
