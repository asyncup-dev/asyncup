...
describe('decodeClaims', () => {
  it('extracts iss/aud without verifying', () => {
    const token = jwt({ iss: 'chat@system.gserviceaccount.com', aud: '742900314218' });
    expect(decodeClaims(token)).toEqual({ iss: 'chat@system.gserviceaccount.com', aud: '742900314218' });
  });
...
describe('classifyFailure', () => {
  ...
  it('names an issuer mismatch', () => {
    const r = classifyFailure({ aud: '742900314218', iss: 'evil@example.com' }, ['742900314218'], 'x');
  ...
  it('falls back to signature/expiry when aud and iss look right', () => {
    const r = classifyFailure(
      { aud: '742900314218', iss: 'chat@system.gserviceaccount.com' },
      ['742900314218'],
      'Token used too late',
    );
...
describe('ChatRequestVerifier', () => {
  it('reports missing/malformed Authorization headers without any network call', async () => {
    const v = new ChatRequestVerifier(['742900314218']);
  ...
  it('accepts either a project number or an app URL as audience', () => {
    ...
    const v = new ChatRequestVerifier(['742900314218', 'https://standup.example.com/chat/events']);
