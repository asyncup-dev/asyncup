...
describe('Scheduler', () => {
  ...
  it('is idempotent: repeated ticks never re-prompt or re-open', async () => {
    const { adapter, scheduler, clock, repo } = makeStack();
    seedStandup(repo);
    clock.set('2026-06-10T09:30');
    await scheduler.tick();
  ...
  it('does not open a run when everyone is on vacation', async () => {
    const { repo, adapter, scheduler, clock } = makeStack();
    const standup = seedStandup(repo);
    for (const p of repo.listParticipants(standup.id)) {
      repo.setParticipantVacation(standup.id, p.userName, true);
    }
    clock.set('2026-06-10T09:30');
    await scheduler.tick();
    expect(repo.getRun(standup.id, '2026-06-10')).toBeNull();
  ...
  it('posts the weekly digest after the last configured day of the week', async () => {
    const { repo, adapter, scheduler, service, clock } = makeStack();
    const standup = seedStandup(repo);
    repo.updateStandup(standup.id, { digestEnabled: true });

    // Wednesday run: closes without a digest
    clock.set('2026-06-10T09:30');
    await scheduler.tick();
    const wed = repo.getRun(standup.id, '2026-06-10')!;
    ...
    await scheduler.tick();
    const fri = repo.getRun(standup.id, '2026-06-12')!;
  ...
  it('posts an AI summary after close when enabled and configured', async () => {
    ...
    const standup = seedStandup(repo);
    repo.updateStandup(standup.id, { aiEnabled: true });
  ...
  it('never posts an AI summary when no one submitted', async () => {
    ...
    const { repo, adapter, scheduler, clock } = makeStack({ summarizer: new AiSummarizer(fakeLlm) });
    const standup = seedStandup(repo);
    repo.updateStandup(standup.id, { aiEnabled: true });
